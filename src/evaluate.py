...
def evaluate_pipeline():
    ...
    for item in dataset:
        ...
        try:
            ...
            if pure_intent_match:
                metrics["pure_correct_intents"] += 1
            if extraction_pure.status == "INVALIDO":
                metrics["pure_critical_errors"] += 1
        ...
        try:
            ...
            if hybrid_intent_match:
                metrics["hybrid_correct_intents"] += 1
            if extraction_hybrid.status == "INVALIDO":
                metrics["hybrid_critical_errors"] += 1
    ...
    print("-" * 80)
    print(
        f"{'Aderência ao Schema':<25} | {calc_percent(metrics['pure_schema_adherence'], total):<25} | {calc_percent(metrics['hybrid_schema_adherence'], total):<25}"
    )
    print(
        f"{'Acurácia de Intenção':<25} | {calc_percent(metrics['pure_correct_intents'], total):<25} | {calc_percent(metrics['hybrid_correct_intents'], total):<25}"
    )
    print(
        f"{'Erros Críticos (INVALIDO)':<25} | {metrics['pure_critical_errors']:<25} | {metrics['hybrid_critical_errors']:<25}"
    )
